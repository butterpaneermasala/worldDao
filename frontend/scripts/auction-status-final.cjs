// Check Auction Status and NFT Minting - CORRECTED
...
async function checkAuctionStatus() {
    try {
        ...
        if (phase === 2) {
            console.log('✅ System is in BIDDING phase - Checking auction...\n');
            ...
            if (isAuctionActive) {
                try {
                    ...
                    console.log(`   Time Left: ${Math.max(timeLeft, 0)} seconds (${Math.max(Math.floor(timeLeft / 60), 0)} minutes)`);
                ...
                console.log('   ❌ Auction is not active despite being in bidding phase');
            }
            ...
            if (isAuctionActive) {
                ...
                console.log('💡 The relayer has successfully performed the upkeep!');
            } else {
                console.log('❌ Auction is in bidding phase but not active - something may be wrong');
            }

        } else if (phase === 1) {
            console.log('⏳ System is in VOTING phase');
            console.log('   The voting phase is still active.');
            console.log('   Need to finalize voting and mint NFT to start auction.');
